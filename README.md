[![Travis CI](https://travis-ci.org/scivision/histfeas.svg?branch=master)](https://travis-ci.org/scivision/histfeas)
[![coverage](https://coveralls.io/repos/scivision/histfeas/badge.svg?branch=master&service=github)](https://coveralls.io/github/scivision/histfeas?branch=master)
...
Running a simulation:
    ...
    ./FigureMaker.py in/my.ini out/myout  # omitting 2nd argument autonames output

Loading a previously-run simulation (changing plot clim for example):

    ./FigureMaker.py --load out/myout

## Simulation Examples

### simulate flaming aurora with two cameras

    ./FigureMaker.py in/2cam_flame.ini out/test_flame2/ -m fwd optim png show h5

you can then look to the Output Processing section for how to load the HDF5 files you just produced in `out/test_flame2`
...
combine the following commands as desired under the `-m` option to control the type of program output
...
* `-m realvid` both cameras in one figure -m singleraw each camera images individually, without axes (for powerpoint,posters, etc.)
...
* `phifwd` this is your "known" input differntial number flux of the
electron precipitation to the simulation (for real data, we don't have
this)
* `phiest` this is the unobservable "unknown" we estimate with this
program (for real and simulated data)
* `pfwd` and `pest` volume emission rate due to simulated / estimated flux
respectively
* `bfwd` and `best` camera optical intensity due to simulated / estimated
flux respectively
...
Other parameters including: simulated camera location, inversion method, and inversion iteration are iterated externally to th HiST program. 
You can use GNU Parallel, or more simply, the methods show
in Examples/ using pure Python parallelism. They will run in parallel on
a PC or high performance computing cluster, outputting to uniquely named
directories. The results are collected and analyzed by the same scripts.
...
simulation parts will be run--disk space use is trivial, so we have left
this alone.

The naming of the variables follows Plot explanation

For Python, we have the hollow function `loadAnalyze.py` which loads the
HDF5 data to call the same `analysehst.py` that's used by the simulation
online--good coding practice.
...
1.  `rawDMCreader.py` accesses the raw camera data and averages the
    selected frames and writes the average as a FITS file
2.  The second line moves this FITS file to the user-selected calibration directory
3.  The third line uses my wrapper and post-processing based on
    Astrometry.net to make an HDF5 file of the mapping from each pixel
    to sky coordinates (ra/dec and az/el).
