...
class Sim:
    def __init__(self, sp, ap, ntimeslice, P):
        ...
        try:
            ...
            if usecamreq[0] is not None:  # override spreadsheet
                ...
                for i, civ in enumerate(
                    self.useCamBool
                ):  # this might be a silly indexing method, but works
                    if (i == usecamreq).any():
            ...
            if usecamreq[0] is not None:
                assert np.all(
                    self.useCamBool == usecamreq
                )  # not .all() in case of different length
        except (TypeError, KeyError):
            pass  # normal case
        ...
        if self.camxreq is not None:
            assert (
                len(self.camxreq) == self.nCamUsed
            ), "must specify same number of x-loc and used cameras"
        #%%

        nCutPix = fromstring(
            sp["cam"]["nCutPix"], dtype=int, sep=","
        )  # FIXME someday allow different # of pixels..
        assert (
            nCutPix[self.useCamBool] == nCutPix[self.useCamBool][0]
        ).all(), "all cameras must have same 1D cut length"
        ...
        self.minflux = sp.getfloat("recon", "minflux", fallback=0.0)
        self.minenergy = sp.getfloat("recon", "minenergy", fallback=0.0)
        #%% reactions
        self.reacreq = []
        ...
        try:  # override
            if P["overrides"]["treq"] is not None:  # must be is not None for array case
                ...
                self.startutc = parse(
                    sp.get("cams", "reqStartUT", fallback=None)
                )  # not fallback=''
    ...
    def setupFwdXZ(self, sp):
        ...
        if self.useztranscar:
            ...
            (Fwd["x"], Fwd["z"]) = makexzgrid(
                self.fwd_xlim, self.fwd_zlim, self.fwd_dxKM, self.fwd_dzKM
            )
        if Fwd["z"] is None:
            raise ValueError(
                "You must specify zmax zmin zcell when not using transcar altitudes in XLS"
            )
    ...
    def maketind(self, timeInds):
        ...
        return timeInds[
            timeInds < self.nTimeSlice
        ]  # (it's <, not <=) slice off commond line requests beyond number of frames
    ...
    def getEllHash(self, sp, x, z):
        EllCritParams = [
            ...
            sp.getfloat("fwd", "XcellKM"),
            sp.getfloat("fwd", "XminKM"),
        ...
        if not self.useztranscar:  # FIXME maybe we should always consider these for best safety
            EllCritParams.extend(
                [
                    sp.getfloat("fwd", "ZcellKM"),
                    sp.getfloat("fwd", "ZminKM"),
                    sp.getfloat("fwd", "ZmaxKM"),
                ]
            )
        ...
        if self.raymap == "arbitrary":
            EllCritParams.extend(
                [
                    fromstring(sp["cam"]["boresightElevDeg"], dtype=float, sep=","),
