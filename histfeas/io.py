...
def planviewkml(cam, xKM, zKM, makeplot, figh, odir):
    ...
    #%% setup LookAt
    lkat = simplekml.LookAt(
        latitude=65.111, longitude=-147.465, altitude=0, heading=180, range=4e3, tilt=45
    )
    ...
    for C in cam:
        if C.usecam:
            ...
            ax.plot(
                C.lon, C.lat, "o", color=clr[C.name], markersize=12, label="cam{}".format(C.name)
            )
...
def KMLline(kml, lla):
    ...
    ls = kml.newlinestring(name="{:.1f} km".format(dist_km), coords=(lla[0][::-1], lla[1][::-1]))
