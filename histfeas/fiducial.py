...
matplotlib.rcParams.update(
    {"font.family": "sans-serif", "font.sans-serif": "Arial", "text.usetex": True}
)
import imageio
...
def fiducial(img, xcrop, ycrop, outfn, rings, rays, t, pstr, oxyfull, wh0, ringmult, axlim=None):
    ...
    """
    assert isinstance(img, ndarray) and img.ndim in (
        2,
        3,
    ), "I expect a single image, greyscale or RGB"
    ...
    #%% init image
    fg = figure()
    ax = Axes(fg, [0.0, 0.0, 1.0, 1.0])  # square
    ax.set_axis_off()
    fg.add_axes(ax)
    if axlim:
        ax.axis(axlim)
    ...
    if rings:
        xyr1 = array(
            (wh0[0] * ringmult[0] / RINGCALDEG - xcrop, wh0[1] * ringmult[0] / RINGCALDEG - ycrop)
        )  # by inspection
        ringincr = wh0[1] * ringmult[0] / RINGCALDEG  # every 2 degrees
        for m in ringmult:
            xyr = m * xyr1 + 1.0

            ax.add_patch(Ellipse(xy=oxy, width=xyr[0], height=xyr[1], fc="none", ec="white", lw=1))

            angtxt(ringincr * m / 2.0, oxy, "{:.0f}$^\circ$".format(m), ax, wh0)
    #%% plot rays
    magzenel = 77.5
    magzenaz = 207.5
    # 1328, 876
    if rays:
        ...
        for r in xyl:
            ax.plot([r[0], oxy[0]], [r[1], oxy[1]], color="red", linestyle="-", lw=1)
    #%% annotate times
    ax.text(
        0.035,
        0.965,
...
def angtxt(radput, oxy, txt, ax, wh0):
    angput = radians(30)
    ax.text(
        wh0[0] / wh0[1] * radput * cos(angput) + oxy[0],
