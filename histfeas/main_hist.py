...
def doASI(P:dict):
    """
    Tomographic inversion for ASI cameras such as DASC and THEMIS
    using xarray as base class instead of mess of custom classes
    """
#%% housekeeping
    logging.basicConfig(level=30-P['verbose']*10)
#%% Step 0) load data
...
#%% synthetic diff. num flux
    Phi0all = getPhi0(sim,arc,Fwd['x'],Peig['Ek'], P['makeplot']) # Nenergy x Nx x Ntime
    print(f'{time()-tic):.1f} sec to prepare for main loop')
#%%start looping for each time slice in keogram (just once if simulated)
    for ti in timeInds:
        logging.info('entering time {}'.format(ti))
        if sim.realdata:
...
#%% wrapup
    msg =f'{argv[0]} program end'; print(msg)#; print(msg,file=stderr)