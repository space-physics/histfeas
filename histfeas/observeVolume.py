...
def getObs(sim, cam, L, tDataInd, ver):
    ...
    if sim.realdata:
        bn = empty(
            nCutPix * sim.nCamUsed, dtype=float, order="F"
        )  # FIXME assumes all cuts same length AND that cam 0 is used
        for C in cam:
            if not C.usecam:
                continue
            """
    ...
    if isnan(bn).any():
        ...
        logging.critical(
            "NaN detected at tInd = {}   dumping variables to {}".format(tDataInd, dumpFN)
        )
...
def makeCamFOVpixelEnds(Fwd, sim, cam, P):
    ...
    for C in cam:
        ...
        if C.usecam:
            ...
            zFOVpixelEnds[:, C.name] = (C.fovmaxlen * sin(radians(C.angle_deg))) + C.alt_m / 1000.0
    ...
    Xpc = append(
        Xpc, Fwd["x"][-1] + 0.5 * sim.fwd_dxKM
    )  # last column by shifting half-cell to the right
    ...
    L = EllLineLength(
        ...
        [c.alt_m / 1000.0 for c in cam if c.usecam],
        nCutPix,
        sim,
        P["makeplot"],
    )
...
def loadEll(sim, Fwd, cam, P):
    try:
        with h5py.File(str(sim.FwdLfn), "r", libver="latest") as fid:
            ...
            if Fwd is not None:  # we're in main program
                if np.any(Fwd["x"] != fid["/Fwd/x"]):  # don't use .any() in case size is different
                    raise ValueError(
                        "need to recompute L, as x-locations arent matched: loaded vs. commanded"
                    )
                if np.any(Fwd["z"] != fid["/Fwd/z"]):  # don't use .any() in case size is different
                    raise ValueError(
                        "need to recompute L, as z-locations arent matched: loaded vs. commanded"
                    )
            elif Fwd is None:  # we're in another program
                Fwd = {"x": fid["/Fwd/x"].value, "z": fid["/Fwd/z"].value}
            else:
                raise TypeError(
                    "I dont understand what you're trying to do with Fwd, which should either be None or properly initialized"
                )
        ...
        logging.error(
            "grid mismatch detected. use --ell command line option to save new Ell file. {}".format(
                e
            )
        )
...
def ind2slice(ind):
    ...
    assert isinstance(
        ind[0], (int, int32, int64)
    ), "only integers are permissable for indexing"  # numpy.nonzero outputs int32 sometmies even on Linux Py3.5
    assert (
        diff(ind) == 1
    ).all(), "only sequential integers for now. Other uniform steps are possible with slight modification"
