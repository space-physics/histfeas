...
def userinput(ini: Path=None, outdir: Path=None) -> Dict[str, Any]:
...
# %% directory handling
    if not P.load and not P.outdir:
        P.outdir = mkdtemp(prefix=Pm['ini'].stem, dir='out')

    if Pm['load'] and not P.outdir:
        if Pm['ini'].is_file():
            P.outdir = Pm['ini'].parent
        elif Pm['ini'].is_dir():
            P.outdir = Pm['ini']
        else:
            raise FileNotFoundError(f"nothing found at {Pm['ini']}")

    Pm['outdir'] = Path(P.outdir).expanduser()
    Pm['outdir'].mkdir(parents=True, exist_ok=True)
# %%
    Pm['overrides']['rootdir'] = Path(__file__).parents[1]

    Pm['overrides']['fwdguess'] = P.fwdguess
    Pm['overrides']['camx'] = P.cx
    Pm['overrides']['fitm'] = P.fitm
    Pm['overrides']['niter'] = P.iter
# %%
...
def setupArc(xl) -> dict:
    ...
    """
    arc: Dict[str, Any] = {}
    ntimeslice: Union[None, int] = None
