...
def userinput(ini=None, outdir=None):
    ...
    p.add_argument(
        "-g", "--fwdguess", help="feed minimizer fwd answer. true | randn stddev |", nargs="+"
    )
    ...
    p.add_argument(
        "-m", "--makeplot", help="plots to make", default=["realvid", "fwd", "optim"], nargs="+"
    )
...
def getParams(P):
    ...
    # store x,z in sim
    ellname = sim.getEllHash(
        xl, [C.x_km for C in cam if C.usecam], [C.alt_m / 1000.0 for C in cam if C.usecam]
    )
...
def cam0dist(cam):
    ...
    """
    cam[0].x_km = 0.0  # NOTE arbitrarily picks the first camera x=0km
    for C in cam[1:]:
        if C.usecam:
    ...
    for s in xl:
        if s.startswith("arc"):
            ...
            if ntimeslice is not None:
                assert (
                    len(texp) == ntimeslice + 1
                ), "for now, all Arcs must have same number of times (columns)"

            # TODO assert all arcs have same time length
            ntimeslice = (
                texp.size - 1
            )  # MUST be -1 to allow for range() compat and interp1() compat
...
def setupCam(sim, cp, zmax, P):
    ...
    if sim.camxreq is not None:
        ...
        for C, cx in zip(
            sim.camnames, sim.camxreq
        ):  # enumerate as in general camera 0 may not be used
            cam.append(
                Cam(sim, cp, C, zmax, xreq=cx, makeplot=P["makeplot"], verbose=P["verbose"])
            )
    else:
        for C in sim.camnames:
            cam.append(Cam(sim, cp, C, zmax, makeplot=P["makeplot"], verbose=P["verbose"]))
